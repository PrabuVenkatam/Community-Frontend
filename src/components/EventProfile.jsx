...
const EventProfile = () => {
    ...
    const handleToggleStatus = async () => {
        try {
            ...
            const response = await toggleEventStatus(event._id || event.id);
            if (response.success) {
                toast.success(response.message);
                setEvent((prev) => ({ ...prev, isActive: response.data.isActive }));
            }
        } catch (err) {
    ...
    if (error || !event) {
        return (
            <div className="flex flex-col items-center justify-center min-h-[400px] text-center p-6 text-secondary">
                <p className="text-lg font-semibold">{error || "Event not found"}</p>
    ...
    const handleAddPosts = async (files) => {
        try {
            ...
            const response = await addEventPost(event._id, formData);
            if (response.success) {
            ...
            <section className="bg-white rounded-[16px] md:rounded-[24px] border border-gray-200 p-4 md:p-6 shadow-sm">
                ...
                <div
                    ...
                    style={{
                        ...
                        backgroundImage: `linear-gradient(to right, rgba(10, 15, 38, 0.95), rgba(10, 15, 38, 0.4)), url('${event.coverImage ? `${BASE_URL}/${event.coverImage}` : 'https://images.unsplash.com/photo-1540575861501-7cf05a4b125a?auto=format&fit=crop&q=80&w=2000'}')`,
                        ...
                        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                            <InfoCard title="Basic Details">
                                <div className="grid grid-cols-2 xl:grid-cols-3 gap-6">
                                    ...
                                    <DataItem label="Registration Type" value={event.registrationType} />
                                    <DataItem label="Reg Start Date" value={event.registrationStartDate ? new Date(event.registrationStartDate).toLocaleDateString() : 'N/A'} />
            ...
            {isAddPostModalOpen && (
                <div className="fixed inset-0 z-50 bg-black/35 flex items-center justify-center p-3">
                    <div className="w-full max-w-[520px] bg-white rounded-[18px] border border-gray-200 shadow-xl p-4 sm:p-5">
                        ...
                        <p className="text-[14px] text-[#6B7280] mb-4">Upload posters for this event.</p>
